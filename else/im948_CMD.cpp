...
F32 AngleX, AngleY, AngleZ,  Ax, Ay, Az,  Gx, Gy, Gz,  Temperature, airPress, Height;// 从Cmd_RxUnpack中获取到的欧拉角数据等更新到全局变量以便用户自己的业务逻辑使用, 若还需要其它数据，可参考进行增加即可
...
static void Cmd_Write(U8 *pBuf, int Len)
{
    Serial1.write(pBuf, Len); // 通过该函数发送通信数据流
